...
// Make vertexIcon larger and easier to grab
...
const MapComponent = forwardRef(
  (
    ...
    // Modal state for grid image preview
          ...
          if (photoMap && photoMap[index] && photoMap[index].photo_base64) {
            ...
            if (!mapInstanceRef.current.getPane("photoBorderPane")) {
              ...
              mapInstanceRef.current.getPane(
                "photoBorderPane"
              ).style.zIndex = 700; // Increased zIndex for border
            }

            // Move image overlay to a higher zIndex pane as well
            if (!mapInstanceRef.current.getPane("photoImagePane")) {
              mapInstanceRef.current.createPane("photoImagePane");
              mapInstanceRef.current.getPane(
                "photoImagePane"
              ).style.zIndex = 690; // Higher than grid polygons, below border
            }

            // Add the image overlay (set pane here, do NOT call setPane)
            ...
            imgOverlay.on("click", () =>
              handleGridImageClick({
                base64: photoMap[index].photo_base64,
                filename: photoMap[index].filename,
                detected: !!photoMap[index].detected,
              })
            );
            ...
            // imgOverlay.bindPopup(borderRect.getPopup()); // <-- Remove this line
    ...
    useEffect(() => {
      if (currentStep === "draw-area") {
        ...
        setConfirmButtonDisabled(
          startPointRef.current === null && pendingStartPoint === null
        );
        setDrawButtonDisabled(true);
      } else {
        setConfirmButtonDisabled(true);
    ...
    const handleGridImageClick = (imgData) => {
      setModalImg(imgData);
      setModalOpen(true);
    };
          ...
          <Paper sx={{ p: 0.5, boxShadow: 2 }}>
            ...
            >
              <Select
                ...
                sx={{ height: 32 }}
                startAdornment={
                  <MapIcon sx={{ mr: 0.5, ml: -0.5, fontSize: 20 }} />
                }
              >
          ...
          (currentStep === "draw-area" ||
            ...
            >
              <Typography variant="body1">
                ...
                {currentStep === "draw-area" &&
                  isDrawing &&
                  "Click on the map to draw the area. Minimum 3 points required. Click Confirm to finish."}
                {currentStep === "set-start" && null}
                {currentStep === "complete" &&
                  "Adjust altitude and overlap settings, then calculate grid coverage."}
              </Typography>
            </Paper>
          )}
