...
const MapComponent = forwardRef(
  (
    ...
    useEffect(() => {
      ...
      if (gridData.grids && gridData.grids.length > 0) {
        ...
        gridData.grids.forEach((grid, index) => {
          ...
          // If a photo exists for this grid index, show it as a popup or marker
          if (photoMap && photoMap[index] && photoMap[index].photo_base64) {
            const center = [grid.center.lat, grid.center.lon];
            // Always use the photo_base64 as-is (it already contains the prefix)
            const imgHtml = `<img src="${photoMap[index].photo_base64}" style="width:100%;height:100%;object-fit:cover;border-radius:4px;" />`;
            ...
            // ...existing code for drawing grid polygon...
            const corners = grid.corners.map((corner) => [
              corner.lat,
              corner.lon,
            ]);
    ...
    // Update drone marker on GPS change
    useEffect(() => {
      if (!mapInstanceRef.current) return;
      if (
        droneGps &&
        typeof droneGps.latitude === "number" &&
        typeof droneGps.longitude === "number"
      ) {
        const lat = droneGps.latitude;
        const lon = droneGps.longitude;
    ...
    return (
      ...
      >
        ...
        />
        {mapInstance && <DroneTracker map={mapInstance} />}
