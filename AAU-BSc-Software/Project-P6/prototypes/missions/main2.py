...
import json
import os
# Add Olympe imports for drone control
...
# Drone configuration
DRONE_IP = "192.168.53.1"  # Default Anafi IP address
...
def execute_flight_plan(waypoints, altitude):
    """
    Execute a flight plan with the Anafi drone
    
    Args:
        waypoints: List of waypoint dictionaries with lat/lon coordinates
        altitude: Flight altitude in meters
    
    Returns:
        Dictionary with flight execution status
    """
    ...
    try:
        ...
        for i, waypoint in enumerate(waypoints):
            ...
            drone(
                moveTo(lat, lon, altitude, orientation_mode.TO_TARGET, 0.0)
            ...
            if wp_type == "grid_center":
                ...
                # Pause briefly to ensure stable photo
                time.sleep(1)
                # Here you would trigger the drone's camera
                # This would require additional Olympe commands specific to camera control
        
        # Return to home (usually the first waypoint)
        ...
        execution_log.append({"action": "disconnect", "timestamp": datetime.datetime.now().isoformat()})
        drone.disconnect()
...
def api_execute_flight():
    """API endpoint to execute flight plan with drone"""
    try:
        data = request.get_json()
