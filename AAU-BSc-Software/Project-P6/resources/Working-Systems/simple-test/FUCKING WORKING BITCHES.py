...
DRONE_READY = False
drone_testing = True  # Set to True by default to enable GPS simulation for indoor testing
gps_data = {
    "latitude": 0,
    "longitude": 0,
    "altitude": 0
...
def handle_flight_execution(sid, data):
    ...
    return {"status": f"indoor test flight started with drone {drone_id}"}
...
def start_background_tasks():
    ...
    def background_loop():
        ...
        while True:
            try:
                if drone_connected:
                    # For indoor testing mode, update simulated GPS data regularly
                    if drone_testing:
                        gps_update_counter += 1
                        if gps_update_counter % 10 == 0:  # Log only every 10th update to reduce spam
