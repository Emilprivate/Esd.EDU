...
class FlightListener(olympe.EventListener):
    ...
    def on_photo_progress(self, event, scheduler):
        ...
        if result.name == 'photo_saved':
            media_id = event.args['media_id']
            media = None
            if media is None:
                assert flight.drone.media_autoconnect
                media = flight.drone.media
                media.download_dir = "photos/"
            flight.drone(download_media(media_id))
    ...
    def _stable(self, wps, alt):
        ...
        for wp in wps:
            ...
            if wp['type']=='grid_center': 
                ...
                time.sleep(0.5)  # Give camera time to switch modes
                d(take_photo(cam_id=0) >> 
