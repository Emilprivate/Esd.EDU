...
const MapComponent = forwardRef(
  (
    ...
    useImperativeHandle(ref, () => ({
      ...
      clearAll: () => {
        if (mapInstanceRef.current) {
          ...
          tempMarkersRef.current.forEach((marker) => mapInstanceRef.current.removeLayer(marker));
          tempPolylinesRef.current.forEach((line) => mapInstanceRef.current.removeLayer(line));
    ...
    useEffect(() => {
      const drawBtn = document.getElementById("start-drawing-btn");
      if (drawBtn) {
        drawBtn.disabled = !hasValidGps || isDrawingRef.current || !connected || !droneConnected;
      }
      ...
      <div className="map-wrapper" style={{ position: "relative", width: "100%", height: "100%" }}>
        ...
        <div
          ...
          style={{
            width: "100%",
            height: "100%",
            filter: !connected || !droneConnected ? "blur(4px) grayscale(0.7)" : "none",
            pointerEvents: !connected || !droneConnected ? "none" : "auto",
            transition: "filter 0.2s",
          }}
        />
        {mapInstance && <DroneTracker map={mapInstance} resetKey={droneTrailResetKey} />}
        ...
        <div className="map-instructions">
          ...
          {flightStep === 'idle' && currentStep === "draw-area" && !isDrawingRef.current && (
            <p>
              {!hasValidGps
