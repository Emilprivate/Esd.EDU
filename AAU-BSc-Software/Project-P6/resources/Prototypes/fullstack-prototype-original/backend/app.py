...
def establish_gps_fix():
    """Establish GPS fix after connecting to the drone"""
    global drone, gps_fix_established
    
    if not drone or not drone_connected:
        return {"success": False, "error": "Drone is not connected"}
    
    try:
        logging.info("Waiting for GPS fix...")
        drone(GPSFixStateChanged(fixed=1, _timeout=10)).wait().success()
        
        gps_fix_established = True
        logging.info("GPS fix established!")
        
        # Get initial GPS position
        update_gps_data()
        
        return {"success": True, "message": "GPS fix established"}
    except Exception as e:
        gps_fix_established = False
        logging.error(f"Failed to establish GPS fix: {str(e)}")
        return {"success": False, "error": str(e)}
    ...
    if drone and drone_connected:
        try:
            ...
            if position_state:
                ...
                print(f"\033[92mGPS Position: lat={gps_data['latitude']:.6f}, lon={gps_data['longitude']:.6f}, alt={gps_data['altitude']:.2f}m\033[0m")
                
                # Add to flight log (only if values have changed)
            ...
            if motion_state:
                ...
                # Convert motion state enum to string to make it JSON serializable
                raw_state = str(motion_state["state"])
                drone_motion_state = raw_state.split('.')[-1].lower()
                
                # Print motion state change in terminal with more details
                if old_state != drone_motion_state:
                    print(f"\033[94mMotion State changed: {old_state} -> {drone_motion_state} (raw: {raw_state})\033[0m")
            ...
            print(f"\033[95mEmitting: {emit_data}\033[0m")
            socketio.emit('gps_update', emit_data)
            
        except Exception as e:
            print(f"\033[91mError updating drone data: {str(e)}\033[0m")
...
def handle_connect():
    ...
    # Only emit to the connecting client, always include motion_state
    socketio.emit('gps_update', {**gps_data, "motion_state": drone_motion_state}, to=request.sid)
    socketio.emit('drone_status', {'connected': drone_connected, 'gps_fix': gps_fix_established}, to=request.sid)
...
def handle_connect_drone(data):
    ...
    socketio.emit('drone_status', {'connected': drone_connected, 'gps_fix': gps_fix_established})
    return result

@socketio.on('establish_gps_fix')
def handle_establish_gps_fix(data):
    result = establish_gps_fix()
    socketio.emit('drone_status', {'connected': drone_connected, 'gps_fix': gps_fix_established})
    return result
...
def gps_update_task():
    """Background task to constantly retrieve GPS data"""
    logging.info("GPS update background task started")
    while True:
        try:
            if drone_connected:
                update_gps_data()
            time.sleep(0.5)  # Update twice per second for more responsiveness
        except Exception as e:
            logging.error(f"Error in GPS update task: {str(e)}")
...
if __name__ == "__main__":
    ...
    logging.info(f"Access the drone control dashboard at http://{host}:{port}")
    
    # Start the GPS update task using a dedicated daemon thread
    # This ensures it keeps running and exits when the main program exits
    gps_thread = threading.Thread(target=gps_update_task)
    gps_thread.daemon = True
    gps_thread.start()
    logging.info("GPS update thread started")
